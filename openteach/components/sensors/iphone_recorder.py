...
class IPhoneCameraRecorder(Recorder):
    ...
    # Recorder base class expects a _recorder_file_name attribute for metadata/logging
    # Use the sensor_name as the base (no extension) so metadata can be associated
    # independent of output file extensions.
    ...
    def _init_rgb_writer(self, frame_bgr):
        if self._rgb_writer is None:
            ...
            print(f"[IPhoneRecorder] Initializing RGB writer {self.rgb_path} size={(w,h)} freq={self.timer.freq}")
    ...
    def _init_depth_writer(self):
        if self._depth_writer is None:
            fourcc = cv2.VideoWriter_fourcc(*"mp4v")
            print(f"[IPhoneRecorder] Initializing Depth writer {self.depth_path} size={(self._depth_w,self._depth_h)} freq={self.timer.freq}")
            ...
            try:
                ...
                try:
                    ...
                    if frame_bgr is not None:
                        print(f"[IPhoneRecorder] Received RGB frame ts={ts_ns}")
                        self._init_rgb_writer(frame_bgr)
                        if self._rgb_writer is not None:
                            self._rgb_writer.write(frame_bgr)
                        else:
                            print("[IPhoneRecorder] Warning: _rgb_writer is None after init")
                    else:
                        print("[IPhoneRecorder] Warning: recv_rgb_image returned None for frame_bgr")
                except Exception:
                    # log exception to aid debugging
                    import traceback
                    print("[IPhoneRecorder] RGB receive/write error:\n", traceback.format_exc())

                # Depth
                try:
                    depth_mm, _ = self.depth_sub.recv_depth_image()
                    if depth_mm is None:
                        print("[IPhoneRecorder] Warning: recv_depth_image returned None")
                    else:
                        ...
                        print(f"[IPhoneRecorder] Saved depth bin: {bin_path}")
                        self._write_depth_visualization(depth_m)
                except Exception:
                    import traceback
                    print("[IPhoneRecorder] Depth receive/write error:\n", traceback.format_exc())
